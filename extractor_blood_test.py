...
class ClinicBloodTestExtractor(BaseBloodTestExtractor):
            ...
            for table in page.find_tables(add_lines=[vertical_line]).tables:
                for row in table.extract():
                    ...
                    if not row or len(row) < 4:
                        logging.warning(
                            "Warning: invalid row found and skipped: %s", row
                        )
                        continue
...
def process_pdf(filepath, file_hash):
    ...
    """
    try:
        return ClinicBloodTestExtractor(filepath, file_hash).process()
